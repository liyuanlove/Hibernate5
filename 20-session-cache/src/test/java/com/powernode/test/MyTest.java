...
public class MyTest {
    ...
    public void test03() {
        ...
        try {
            ...
            /*
             * 当执行删除操作时，并不会直接执行delete的SQL语句，
             * 但会将session缓存map中该被删除对象的key所对应的value置为null，
             * 不过这个key所在的entry并没有被删除，即该对象的key仍存在于map中。
             * 为什么要将value值删除而保留key？
             * 因为到达同步点后，要将session中的数据与数据库中的数据进行同步，就要删除数据库中的该对象，
             * 而对数据库中该对象的删除操作，需要根据id进行，所以不删除map中的id，
             * 而在到达刷新点后，才会执行delete的SQL语句。
    ...
    public void test04() {
        ...
        try {
            ...
            Student student = session.get(Student.class, 2);
            /*
             * 当执行修改操作时，即使到达刷新点，是否执行update的SQL语句，
             * 还要取决于修改的数据与快照中数据的对比结果：
             * 相同，则不执行；否则，执行update的SQL语句。
             */
            student.setName("李四");
